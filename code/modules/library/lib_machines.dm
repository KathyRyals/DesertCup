...
/obj/machinery/computer/libraryconsole/bookmanagement/Topic(href, href_list)
			...
			if(scanner.cache)
				...
				if(choice == "Confirm")
					if (!SSdbcore.Connect())
						...
						var/datum/db_query/query_library_upload = SSdbcore.NewQuery("INSERT INTO [format_table_name("library")] (author, title, content, category, ckey, datetime, round_id_created) VALUES ('[sqlauthor]', '[sqltitle]', '[sqlcontent]', '[sqlcategory]', '[usr.ckey]', Now(), '[GLOB.round_id]')")
	...
	if(href_list["targetid"])
		...
		if(cooldown > world.time)
			...
			var/datum/db_query/query_library_print = SSdbcore.NewQuery("SELECT * FROM [format_table_name("library")] WHERE id=[sqlid] AND isnull(deleted)")
